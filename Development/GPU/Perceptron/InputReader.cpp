...
vector<vector<float>> InputReader::getTestImageFloatData() {
    vector<vector<float>> testImagesFloatData(testImagesData.size());
    for (int i = 0; i < testImagesData.size(); i++) {
        for (int j = 0; j < testImagesData[i].size(); j++) {
            testImagesFloatData[i].push_back(float(testImagesData[i][j]));
        }
    }
    return testImagesFloatData;
}
